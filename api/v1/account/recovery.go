...
import (
	...
	recovery_services "github.com/merema-uit/server/services/recovery"
...
func RecoveryHandler(ctx *gin.Context) {
	...
	err := recovery_services.SendRecoveryEmail(ctx, req)
	if err != nil {
		switch err {
...
// @Failure 401
// @Failure 500
// @Router /accounts/recovery/verify [post]
func RecoveryConfirmHandler(ctx *gin.Context) {
	...
	token, err := recovery_services.VerifyRecoveryOTP(ctx, req)
	if err != nil {
		switch err {
...
func ResetPasswordHandler(ctx *gin.Context) {
	...
	err := recovery_services.ResetPassword(ctx, req, authHeader)
	if err != nil {
		switch err {
